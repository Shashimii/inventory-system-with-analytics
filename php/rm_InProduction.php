...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['item_desc']) && ($_POST['item_id']) && ($_POST['item_lot']) && ($_POST['item_bin']) && ($_POST['quantity_receive'])) {
        ...
        $stmt->bind_param('ss', $rm_id, $dataStatusInProduction);
        $stmt->execute();
        ...
        if ($num_rows > 0) {
            ...
            $stmt->bind_param("sssssssiss", $sys_date, $sys_time, $sys_user, $rm_desc, $rm_id, $rm_batch, $rm_bin, $rm_quantity, $dataStatusInProduction, $dataActive);
            if ($stmt->execute()) {
                ...
                $stmt->bind_param("sss", $dataNotActive, $rm_id, $dataStatusReceived);
                if($stmt->execute()) {
                    echo '0';
                } else {
                    // sql error
