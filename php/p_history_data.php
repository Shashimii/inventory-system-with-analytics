...
if ($result1->num_rows > 0) {
    while ($row = $result1->fetch_assoc()) {
        ...
        $stmt = $con->prepare("SELECT action_date, action_time, action_by, pack_small, pack_medium, pack_large, shipped_quantity, client_company, quantity_IN, quantity_OUT FROM `products_data` WHERE item_id = ? AND item_name = ? ORDER BY item_id");
