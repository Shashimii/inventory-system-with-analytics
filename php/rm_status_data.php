...
$stmt = $con->prepare("SELECT item_desc,
    ...
    SUM(CASE WHEN (item_data_active = 'Y') THEN quantity_scrap ELSE 0 END) AS total_scrap,
    SUM(CASE WHEN (item_data_active = 'Y' OR item_data_active = 'N') THEN quantity_used ELSE 0 END) AS total_used,
    SUM(CASE WHEN (item_data_active = 'Y') THEN quantity_created_pcs ELSE 0 END) AS total_pcs
    FROM rm_data
    GROUP BY item_desc
    ORDER BY id DESC");
$stmt->execute();
