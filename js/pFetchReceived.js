        ...
        if (searchKey !== '') { 
            switch (searchFilter) { 
                ...
                case 'bin':
                    ...
                    break;
                default:
                    filteredResponse = tableData.filter(item => `${item.item_id} ${item.item_name} ${item.item_desc} ${item.item_lot} ${item.item_bin} ${item.action_date}`.toLocaleLowerCase().includes(searchKey.toLowerCase()));
                    break;
            };
        };
            ...
            pageData.forEach(function(item) { 
                $('#receiveTable tbody').append( 
                    '<tr class="">' +
                        ...
                        '<td class="text-center">' + (item.pack_small ? item.pack_small + ' pcs': '') + '</td>' +
                        '<td class="text-center">' + (item.pack_medium ? item.pack_medium + ' pcs': '') + '</td>' +
                        '<td class="text-center">' + (item.pack_large ? item.pack_large + ' pcs': '') + '</td>' +
